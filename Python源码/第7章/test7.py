...
# 2.在BankAccount类中定义一个名为deposit（存款）的方法，该方法接受一个金额money作为参数，并将其添加到余额中。
# 3.定义另一个名为withdraw（取款）的方法，该方法也接受一个金额money作为参数。如果要取款的金额大于余额，打印"余额不足"，否则从余额中扣除该金额。
# 4.定义一个名为check（查看余额）的方法，打印出当前的余额。
...
class BankAccount:
    ...
    def deposit(self):
        ...
        self.balance+=money
        print(f'存款成功，账户余额：{self.balance}')
    def withdraw(self):
        ...
        if self.balance<money:
            ...
            self.balance-=money
            print(f'取款成功，账户余额：{self.balance}')
    def check(self):
